...
if ( !isset( $current_site ) || !isset( $current_blog ) ) {
	...
	if ( false !== strpos( $domain, ':' ) ) {
		if ( substr( $domain, -3 ) == ':80' ) {
			...
			wp_die( /*WP_I18N_NO_PORT_NUMBER*/'אי אפשר להקים רשת בלוגים עם מספר Port בכתובת'/*/WP_I18N_NO_PORT_NUMBER*/ );
		}
	}
	...
	if ( ! $blog_id ) {
		if ( defined( 'WP_INSTALLING' ) ) {
			...
			$msg = ! $wpdb->get_var( "SHOW TABLES LIKE '$wpdb->site'" ) ? ' ' . /*WP_I18N_TABLES_MISSING*/'חסרות טבלאות בבסיס הנתונים.'/*/WP_I18N_TABLES_MISSING*/ : '';
			wp_die( /*WP_I18N_NO_BLOG*/'אין כאן בלוג בשם הזה.'/*/WP_I18N_NO_BLOG*/ . $msg );
		}
	}
}
