...
function wpmu_current_site() {
	...
	// Still no dice.
	if ( 1 == count( $sites ) )
		wp_die( sprintf( /*WP_I18N_BLOG_DOESNT_EXIST*/'אין פה בלוג כזה. אפשר לנסות: <a href="%s">%s</a>.'/*/WP_I18N_BLOG_DOESNT_EXIST*/, $sites[0]->domain . $sites[0]->path ) );
	else
		wp_die( /*WP_I18N_NO_SITE_DEFINED*/'אין פה התקנה של וורדפרס (No site defined on this host). אם זה האתר שלך, אפשר למצוא מידע נוסף בעמוד <a href="http://codex.wordpress.org/Debugging_a_WordPress_Network">Debugging a WordPress Network</a> בקודקס.'/*/WP_I18N_NO_SITE_DEFINED*/ );
}
...
function ms_not_installed() {
	...
	$title = /*WP_I18N_FATAL_ERROR*/'לא הצלחתי להתחבר לבסיס הנתונים (Error establishing database connection)'/*/WP_I18N_FATAL_ERROR*/;
	$msg  = '<h1>' . $title . '</h1>';
	if ( ! is_admin() )
		die( $msg );
	$msg .= '<p>' . /*WP_I18N_CONTACT_OWNER*/'אם הבלוג שלך לא מוצג כהלכה, אפשר לנסות לברר את זה עם הנהלת הרשת.'/*/WP_I18N_CONTACT_OWNER*/ . '';
	$msg .= ' ' . /*WP_I18N_CHECK_MYSQL*/'אם זו הרשת שלך, כדאי לבדוק ש-MySQL פועל כהלכה ושאין שגיאות באף אחת מהטבלאות.'/*/WP_I18N_CHECK_MYSQL*/ . '</p>';
	if ( false && !$wpdb->get_var( "SHOW TABLES LIKE '$wpdb->site'" ) )
		$msg .= '<p>' . sprintf( /*WP_I18N_TABLES_MISSING_LONG*/'<strong>חסרות טבלאות בבסיס הנתונים.</strong> זה אומר ש-MySQL לא פועל, שוורדפרס לא הותקנה כהלכה, או שהטבלאות הבאות פשוט נמחקו: <code>%s</code>. כדאי לבדוק מה קורה עם בסיס הנתונים שלך, <strong>בהקדם האפשרי</strong>.'/*/WP_I18N_TABLES_MISSING_LONG*/, $wpdb->site ) . '</p>';
	else
		$msg .= '<p>' . sprintf( /*WP_I18N_NO_SITE_FOUND*/'<strong>לא מצאתי את הבלוג <code>%1$s</code>.</strong> חיפשתי את הטבלה <code>%2$s</code> בבסיס הנתונים <code>%3$s</code>. זה נשמע נכון?'/*/WP_I18N_NO_SITE_FOUND*/, rtrim( $domain . $path, '/' ), $wpdb->blogs, DB_NAME ) . '</p>';
	$msg .= '<p><strong>' . /*WP_I18N_WHAT_DO_I_DO*/'מה לעשות עכשיו?'/*/WP_I18N_WHAT_DO_I_DO*/ . '</strong> ';
	$msg .= /*WP_I18N_RTFM*/'ההנחיות בעמוד <a target="_blank" href="http://codex.wordpress.org/Debugging_a_WordPress_Network">bug report</a> בקודקס עשויות לעזור לך להבין מה קרה כאן.'/*/WP_I18N_RTFM*/;
	$msg .= ' ' . /*WP_I18N_STUCK*/'כדאי גם לבדוק שהטבלאות הבאות קיימות בבסיס הנתונים שלך:'/*/WP_I18N_STUCK*/ . '</p><ul>';
