...
class wpdb {
	...
	function set_prefix( $prefix, $set_table_names = true ) {
		...
		if ( preg_match( '|[^a-z0-9_]|i', $prefix ) )
			return new WP_Error('invalid_db_prefix', /*WP_I18N_DB_BAD_PREFIX*/'קידומת בסיס נתונים בקובץ ההגדרות אינה חוקית'/*/WP_I18N_DB_BAD_PREFIX*/);
	...
	function select( $db, $dbh = null ) {
		...
		if ( !@mysql_select_db( $db, $dbh ) ) {
			$this->ready = false;
			$this->bail( sprintf( /*WP_I18N_DB_SELECT_DB*/'<h1>לא ניתן לגשת לבסיס הנתונים</h1>
...
<p>אם כל זה לא אומר לך כלום, כדאי <strong>לפנות לתמיכה של ספקית האחסון</strong> בטלפון או באימייל. לחלופין, תמיד אפשר לפנות <a href="http://wordpress.org/support/">לקבוצת הדיון של וורדפרס בעברית</a>.</p>'/*/WP_I18N_DB_SELECT_DB*/, $db, $this->dbuser ), 'db_select_fail' );
			return;
		}
	}
	...
	function db_connect() {
		...
		if ( !$this->dbh ) {
			$this->bail( sprintf( /*WP_I18N_DB_CONN_ERROR*/'<div dir="rtl" style="direction:rtl; text-align:right;">
...
</div>'/*/WP_I18N_DB_CONN_ERROR*/, $this->dbhost ), 'db_connect_fail' );
