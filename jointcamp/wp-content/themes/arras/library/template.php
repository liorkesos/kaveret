...
function arras_document_title() {
    ...
    elseif ( is_search() ) { 
      $content = __('תוצאות חיפש:', 'arras'); 
      $content .= ' ' . esc_html(stripslashes(get_search_query()), true);
    }
    elseif ( is_category() ) {
      $content = __('ארכיון קטגוריה:', 'arras');
      $content .= ' ' . single_cat_title("", false);;
    }
    elseif ( is_tag() ) { 
      $content = __('ארכיון תאגים:', 'arras');
      $content .= ' ' . arras_tag_query();
    }
    elseif ( is_404() ) { 
      $content = __('לא נמצא', 'arras'); 
    }
    else { 
      $content = get_bloginfo('description');
    }
	...
	<li <?php comment_class(); ?> id="li-comment-<?php comment_ID() ?>">
		<div class="comment-node" id="comment-<?php comment_ID(); ?>">
			...
			<?php if ( $comment->comment_approved == '0' ) : ?>
				<span class="comment-moderation"><?php _e('התגובה שלך מחכה לאישור.', 'arras') ?></span>	
			<?php endif; ?>
			<div class="comment-meta commentmetadata">
				<?php printf( __('פורסם %1$s ב %2$', 'arras'), '<abbr class="comment-datetime" title="' . get_comment_time( __('c', 'arras') ) . '">' . get_comment_time( __('F j, Y', 'arras') ), get_comment_time( __('g:i A', 'arras') ) . '</abbr>' ); ?>
			</div>
...
?>
	<ul class="quick-nav clearfix">
		...
		<?php if ($twitter_username != '') : ?>
			<li><a id="twitter" title="<?php printf( __( '%s טוויטר', 'arras' ), esc_html( get_bloginfo('name'), 1 ) ) ?>" href="http://www.twitter.com/<?php echo $twitter_username ?>/" target="_blank"><?php _e('טוויטר', 'arras') ?></a></li>
		<?php endif ?>
		
		<?php $facebook_profile = arras_get_option('facebook_profile'); ?>
		<?php if ($facebook_profile != '') : ?>
			<li><a id="facebook" title="<?php printf( __( '%s פייסבוק', 'arras' ), esc_html( get_bloginfo('name'), 1 ) ) ?>" href="<?php echo $facebook_profile ?>" target="_blank"><?php _e('פייסבוק', 'arras') ?></a></li>
		<?php endif ?>
