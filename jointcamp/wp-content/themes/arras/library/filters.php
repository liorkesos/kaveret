...
function arras_postheader() {
	...
	if ( !is_page() ) {
		...
		if ( arras_get_option('post_author') ) {
			$postheader .= sprintf( __('<div class="entry-author">נכתב על ידי %s</div>', 'arras'), '<address class="author vcard"><a class="url fn n" href="' . get_author_posts_url( get_the_author_meta('ID') ) . '" title="' . esc_attr(get_the_author()) . '">' . get_the_author() . '</a></address>' );
		}
		...
		if (current_user_can('edit_post')) {
			$postheader .= '<a class="post-edit-link" href="' . get_edit_post_link($id) . '" title="' . __('ערוך פוסט', 'arras') . '">' . __('(ערוך פוסט)', 'arras') . '</a>';
		}
		...
		if ( !is_attachment() && arras_get_option('post_cats') ) {
			...
			$postheader .= sprintf( __('<span class="entry-cat"><strong>פורסם בקטגוריות: </strong>%s</span>', 'arras'), implode(', ', $post_cats) );
		}
...
function arras_postfooter() {
	...
	if ( arras_get_option('post_tags') && !is_attachment() && is_array(get_the_tags()) )
			$postfooter .= '<div class="tags"><strong>' . __('תאגים:', 'arras') . '</strong>' . get_the_tag_list(' ', ', ', ' ') . '</div>';
...
function arras_post_notfound() {
	...
	$postcontent .= '<h1 class="entry-title">' . __('הדבר הזה שחיפשת, אז זה לא פה!', 'arras') . '</h2>';
	$postcontent .= '<div class="entry-content"><p>' . __('<strong>אנחנו מתנצלים, הדף שאתה מחפש כנראה עף למקום אחר.</strong>', 'arras') . '</p>';
	
	
	$postcontent .= '<form method="get" class="clearfix" action="' . get_bloginfo('url') . '">
	' . __('אולי לחפש את זה יעזור?', 'arras') . '<br />
	<input type="text" value="" name="s" class="s" size="30" onfocus="this.value=\'\'" />
	<input type="submit" class="searchsubmit" value="' . __('חיפוש', 'arras') . '" title="' . sprintf( __('חיפוש %s', 'arras'), wp_specialchars( get_bloginfo('name'), 1 ) ) . '" />
	</form>';
	...
	$output = '
		<div class="about-author clearfix">
			...
			<div class="author-meta">
				<h4>' . sprintf(__('מידע על %s', 'arras'),  get_the_author_meta('display_name')) . '</h4>
