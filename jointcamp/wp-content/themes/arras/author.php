...
<div class="author-content">
	...
	<h1 class="entry-title"><?php printf( __('לגבי המחבר: %s', 'arras'), '<span class="vcard"><a class="url fn n" href="' . get_author_posts_url( get_the_author_meta( 'ID' ) ) . '" title="' . esc_attr(get_the_author()) . '" rel="me">' . get_the_author_meta('display_name') . '</a></span>' ); ?></h1>

	<div class="entry-content clearfix">
		<a href="<?php echo get_author_posts_url(get_the_author_meta('ID')) ?>"><?php echo get_avatar(get_the_author_meta('ID'), 96) ?></a>
		<dl>
			<?php if (get_the_author_meta('user_url')) : ?>
				<dt><?php _e('אתר', 'arras') ?></dt>
				<dd><a href="<?php the_author_meta('user_url') ?>"><?php the_author_meta('user_url') ?></a></dd>
			<?php endif ?>
			<?php if (get_the_author_meta('description')) : ?>
				<dt><?php _e('תיאור', 'arras') ?></dt>
				<dd><?php the_author_meta('description') ?></dd>
			<?php endif ?>
		</dl>
	</div>
	
	<h2 class="author-posts-title"><?php printf( __('פוסטים שנכתבו על ידי %s', 'arras'), '<span class="vcard"><a class="url fn n" href="' . get_author_posts_url( get_the_author_meta( 'ID' ) ) . '" title="' . esc_attr(get_the_author()) . '" rel="me">' . get_the_author_meta('display_name') . '</a></span>' ); ?></h2>
