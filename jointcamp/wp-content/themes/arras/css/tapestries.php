...
if (!function_exists('arras_tapestry_traditional')) {
	function arras_tapestry_traditional($dep = '', $taxonomy) {
		...
		<div <?php arras_single_post_class() ?>>
			...
			<div class="entry-content clearfix"><?php the_content( __('<p>קרא את המשך הפוסט &raquo;</p>', 'arras') ); ?></div>
			<?php arras_postfooter() ?>
		</div>
		<?php
	}
	arras_add_tapestry( 'traditional', __('מסורתי', 'arras'), 'arras_tapestry_traditional', array(
...
if (!function_exists('arras_tapestry_line')) {
	function arras_tapestry_line($dep = '', $taxonomy) {
		...
		<li <?php arras_post_class() ?>>
			...
			<h3 class="entry-title"><a rel="bookmark" href="<?php the_permalink() ?>" title="<?php printf( __('לינק קבוע ל %s', 'arras'), get_the_title() ) ?>"><?php the_title() ?></a></h3>
			<span class="entry-comments"><?php comments_number() ?></span>
		</li>
		<?php
	}
	arras_add_tapestry( 'line', __('בשורה', 'arras'), 'arras_tapestry_line', array(
		...
		<li <?php arras_post_class() ?>>
			...
			<div class="entry-summary">
				<div class="entry-info">
					<abbr class="published" title="<?php the_time('c') ?>"><?php printf( __('פורסם בתאריך %s', 'arras'), get_the_time(get_option('date_format')) ) ?></abbr> | <span><?php comments_number() ?>
<br>
...
<address class="author vcard"><a class="url fn n" href="' . get_author_posts_url( get_the_author_meta('ID') ) . '" title="' . esc_attr(get_the_author()) . '">' . get_the_author() . '</a></address>
				...
				<p class="quick-read-more"><a href="<?php the_permalink() ?>" title="<?php printf( __('לינק קבוע ל %s', 'arras'), get_the_title() ) ?>">
				<?php _e('קרא עוד...', 'arras') ?>
				</a></p>
			</div>	
		</li>
		<?php
	}
	arras_add_tapestry( 'quick', __('תצוגה מהירה', 'arras'), 'arras_tapestry_quick', array(
