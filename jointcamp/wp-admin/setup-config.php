...
<title>וורדפרס &rsaquo; יצירת קובץ הגדרות</title>
...
<p>ברוכים הבאים לוורדפרס. לפני ההתקנה, יש להזין מידע על בסיס הנתונים. כדאי לברר את זה <em>לפני</em> שממשיכים.</p>
<ol>
	<li>שם בסיס הנתונים</li>
	<li>שם משתמש בבסיס הנתונים</li>
	<li>סיסמת המשתמש בבסיס הנתונים</li>
	<li>הכתובת (host) של בסיס הנתונים</li>
	<li>קידומת לטבלאות (כדי להפעיל מספר התקנות וורדפרס מבסיס נתונים אחד)</li>
</ol>
<p><strong>אם אניצירת הקובץ נכשלת - אין מה לדאוג. אפשר לפתוח את הקובץ <code>wp-config-sample.php</code> בכל עורך טקסט, למלא את הנתונים לפי ההוראות, ולשמור אותו בשם <code>wp-config.php</code>. <a href="http://codex.wordpress.org/Editing_wp-config.php">מידע נוסף על יצירת קובץ ההגדרות</a>.</strong></p>
<p>ברוב המקרים, חברת האכסון על שרתיה יושב האתר, תספק פרטים האלה.</p>

<p class="step"><a href="setup-config.php?step=1<?php if ( isset( $_GET['noapi'] ) ) echo '&amp;noapi'; ?>" class="button">אפשר להתחיל!</a></p>
<?php
	break;
...
<form method="post" action="setup-config.php?step=2">
	...
	<table class="form-table">
		<tr>
			...
			<td>שם בסיס הנתונים של וורדפרס.</td>
		</tr>
		<tr>
			<th scope="row"><label for="uname">שם משתמש</label></th>
			<td><input name="uname" id="uname" type="text" size="25" value="username" /></td>
			<td>שם משתמש עם הרשאות מלאות בבסיס הנתונים הזה.</td>
		</tr>
		<tr>
			<th scope="row"><label for="pwd">סיסמה</label></th>
			<td><input name="pwd" id="pwd" type="text" size="25" value="password" /></td>
			<td>סיסמת המשתמש בבסיס הנתונים.</td>
		</tr>
		<tr>
			<th scope="row"><label for="dbhost">כתובת (hostname)</label></th>
			<td><input name="dbhost" id="dbhost" type="text" size="25" value="localhost" /></td>
			<td>הכתובת של בסיס הנתונים. ברוב מהמקרים לא צריך לשנות את זה.</td>
		</tr>
		<tr>
			<th scope="row"><label for="prefix">קידומת לטבלאות</label></th>
			<td><input name="prefix" id="prefix" type="text" id="prefix" value="wp_" size="25" /></td>
			<td>כדי להפעיל כמה התקנות של וורדפרס מבסיס נתונים אחד, אפשר להגדיר לכל התקנה קידומת טבלאות אחרת. מומלץ לשנות קידומת הסטנדרטית.</td>
		</tr>
	</table>
...
<?php
	...
	if ( preg_match( '|[^a-z0-9_]|i', $prefix ) )
		wp_die( /*WP_I18N_BAD_PREFIX*/'<strong>בעיה</strong>: אפשר להשתמש רק באותיות, מספרים וקווים תחתונים עבור "Table Prefix".'/*/WP_I18N_BAD_PREFIX*/ );
	...
	if ( ! is_writable(ABSPATH) ) :
		display_header();
?>
<p>שגיאה - אי אפשר לכתוב לקובץ <code>wp-config.php</code>.</p>
<p>אפשר ליצור את הקובץ <code>wp-config.php</code> ידנית ולהדביק את הטקסט לתוכו.</p>
<textarea cols="98" rows="15" class="code"><?php
		foreach( $configFile as $line ) {
			echo htmlentities($line, ENT_COMPAT, 'UTF-8');
		}
?></textarea>
<p>כשהקובץ יהיה מוכן, אפשר ללחוץ על "התחל התקנה".</p>
<p class="step"><a href="install.php" class="button">התחל התקנה</a></p>
<?php
	else :
		...
		display_header();
?>
<p>סיימנו עם ההכנות! עכשיו ניתן להתחבר לבסיס הנתונים ולהתקין את וורדפרס.</p>
